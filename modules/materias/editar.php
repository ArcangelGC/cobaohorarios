<?php
// modules/materias/editar.php - Editar materia existente
session_start();
...
$materia = obtenerRegistro($sql_materia, [$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $actualizar_horario = isset($_POST['actualizar_horario']) ? 1 : 0;
    ...
    if (empty($nombre)) {
        ...
        // Verificar que no exista otra materia con la misma clave
        $sql_verificar = "SELECT id FROM materias WHERE clave = ? AND id != ?";
        $existe = obtenerRegistro($sql_verificar, [$clave, $id]);
        
        if ($existe) {
            $mensaje = 'Ya existe otra materia con esa clave';
            $tipo_mensaje = 'danger';
        } else {
            // Actualizar la materia (SIN horas_semana)
            $sql = "UPDATE materias SET 
                    ...
                    creditos = ?
                    WHERE id = ?";
            $params = [$nombre, $clave, $semestre_id, $tipo_materia, $especialidad_id, $nucleo_id, $creditos, $id];
            $types = "ssisiiii";
            
            $resultado = ejecutarUpdate($sql, $params, $types);
            
            if ($resultado !== false) {
                // Actualizar horarios si se solicitó
                if ($actualizar_horario && !empty($dias) && $maestro_id > 0 && $salon_id > 0) {
                    // Validar horarios
                    $dias_validos = [];
                    $error_horario = false;
                    ...
                    if (!$error_horario && count($dias_validos) > 0) {
                        ...
                        $sql_delete = "UPDATE horarios SET activo = 0 WHERE materia_id = ?";
                        ejecutarUpdate($sql_delete, [$id]);
                        
                        // Buscar o crear bloque de horario
                        $sql_buscar_bloque = "SELECT id FROM bloques_horarios WHERE materia_id = ? LIMIT 1";
                        $bloque_existente = obtenerRegistro($sql_buscar_bloque, [$id]);
                        
                        if ($bloque_existente) {
                            $bloque_id = $bloque_existente['id'];
                            // Actualizar semanas
                            $sql_update_bloque = "UPDATE bloques_horarios SET semanas = 16 WHERE id = ?";
                            ejecutarUpdate($sql_update_bloque, [$bloque_id]);
                        } else {
                            // Crear nuevo bloque
                            $sql_bloque = "INSERT INTO bloques_horarios (grupo_id, materia_id, semanas) VALUES (NULL, ?, 16)";
                            $bloque_id = ejecutarUpdate($sql_bloque, [$id]);
                        }
                        ...
                        if ($bloque_id) {
                            ...
                            foreach ($dias_validos as $dia) {
                                ...
                                $params_horario = [$bloque_id, $id, $maestro_id, $salon_id, 
                                                   $dia, $horas_inicio[$dia], $horas_fin[$dia]];
                                
                                $resultado_horario = ejecutarUpdate($sql_horario, $params_horario);
                                if ($resultado_horario) $insertados++;
                            }
                            
                            if ($insertados > 0) {
                                $_SESSION['mensaje'] = "Materia actualizada con $insertados dias de horario";
                ...
                // Si no se actualizaron horarios o no se solicitó
                $_SESSION['mensaje'] = 'Materia actualizada exitosamente';
                $_SESSION['tipo_mensaje'] = 'success';
                header('Location: index.php');
                exit;
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <small>Modifica los datos de la materia y sus horarios</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body p-4">
                    ...
                    <form method="POST" action="" id="formMateria">
                        ...
                        <div class="row g-3 mt-1">
                                ...
                                <select class="form-select" id="semestre_id" name="semestre_id" required>
                                    ...
                                    <?php foreach ($semestres as $sem): ?>
                                        <option value="<?php echo $sem['id']; ?>" 
                                            <?php echo $materia['semestre_id'] == $sem['id'] ? 'selected' : ''; ?>>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="tipo_materia" name="tipo_materia" required>
                                    <option value="tronco" <?php echo $materia['tipo_materia'] == 'tronco' ? 'selected' : ''; ?>>Tronco Comun</option>
                                    <option value="especialidad" <?php echo $materia['tipo_materia'] == 'especialidad' ? 'selected' : ''; ?>>Especialidad</option>
                                    <option value="nucleo" <?php echo $materia['tipo_materia'] == 'nucleo' ? 'selected' : ''; ?>>Nucleo</option>
                                </select>
                                <div class="form-text" id="info_tipo">
                                    <?php 
                                    if ($materia['tipo_materia'] == 'tronco') echo 'Materia del tronco comun';
                                    elseif ($materia['tipo_materia'] == 'especialidad') echo 'Materia de especialidad';
                                    else echo 'Materia de nucleo';
                                    ?>
                                </div>
                            </div>
                        ...
                        <div class="row g-3 mt-1">
                            <div class="col-md-6" id="div_especialidad" style="<?php echo $materia['tipo_materia'] == 'especialidad' ? 'display:block' : 'display:none'; ?>">
                                <label for="especialidad_id" class="form-label">Especialidad</label>
                                <select class="form-select" id="especialidad_id" name="especialidad_id">
                                    <option value="">Seleccionar especialidad</option>
                                    <?php foreach ($especialidades as $esp): ?>
                                        <option value="<?php echo $esp['id']; ?>" 
                                            <?php echo $materia['especialidad_id'] == $esp['id'] ? 'selected' : ''; ?>>
                            ...
                            <div class="col-md-6" id="div_nucleo" style="<?php echo $materia['tipo_materia'] == 'nucleo' ? 'display:block' : 'display:none'; ?>">
                                ...
                                <select class="form-select" id="nucleo_id" name="nucleo_id">
                                    ...
                                    <?php foreach ($nucleos as $nuc): ?>
                                        <option value="<?php echo $nuc['id']; ?>" 
                                            <?php echo $materia['nucleo_id'] == $nuc['id'] ? 'selected' : ''; ?>>
                        ...
                        <?php if (count($horarios) > 0): ?>
                            <div class="horarios-actuales mb-3">
                                <h6 class="mb-2">
                                    <i class="fas fa-calendar-alt text-primary"></i> Horarios Actuales
                                    <span class="badge bg-secondary ms-2"><?php echo count($horarios); ?> horas</span>
                                </h6>
                                ...
                                <small class="text-muted d-block mt-2">
                                    <i class="fas fa-info-circle"></i> Los horarios actuales seran reemplazados si los actualizas
                                </small>
                            </div>
                        <?php endif; ?>
                        
                        <!-- Switch para actualizar horario -->
                        <div class="switch-container" id="switchContainer" onclick="toggleSwitch()">
                            <div class="switch">
                                <input type="checkbox" id="actualizar_horario" name="actualizar_horario" value="1">
                                <span class="slider"></span>
                            </div>
                            <div class="switch-label">
                                Actualizar horarios de la materia
                                <small>Reemplaza los horarios actuales por los nuevos</small>
                            </div>
                        </div>
                        ...
                        <div id="seccion_horario" style="display: none;" class="horario-section mt-3">
                                ...
                                <table class="table horario-table">
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($dias_semana as $dia):
                                            // Verificar si este dia tiene horario actual
                                            $tiene_horario = false;
                                            $hora_inicio_actual = '';
                                            $hora_fin_actual = '';
                                            foreach ($horarios as $h) {
                                                if ($h['dia_semana'] == $dia) {
                                                    $tiene_horario = true;
                                                    $hora_inicio_actual = substr($h['hora_inicio'], 0, 5);
                                                    $hora_fin_actual = substr($h['hora_fin'], 0, 5);
                                                    break;
                                                }
                                            }
                                        ?>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <input type="checkbox" name="dias[]" value="<?php echo $dia; ?>" 
                                                       ...
                                                       <?php echo $tiene_horario ? 'checked' : ''; ?>
                                            ...
                                            <td>
                                                <input type="time" class="form-control" 
                                                       ...
                                                       value="<?php echo $hora_inicio_actual; ?>"
                                                       <?php echo $tiene_horario ? '' : 'disabled'; ?>>
                                            </td>
                                            <td>
                                                <input type="time" class="form-control" 
                                                       name="horas_fin[<?php echo $dia; ?>]" 
                                                       id="fin_<?php echo $dia; ?>"
                                                       value="<?php echo $hora_fin_actual; ?>"
                                                       <?php echo $tiene_horario ? '' : 'disabled'; ?>>
                                            </td>
                                        </tr>
                                        <?php endforeach; ?>
                            ...
                            <div class="row g-3 mt-2">
                                <div class="col-md-6">
                                    <div class="dias-counter">
                                        ...
                                        Dias seleccionados: 
                                        <span class="badge bg-primary" id="contador_dias"><?php echo count($horarios); ?></span>
                                    </div>
                                </div>
                            ...
                            <!-- Vista previa -->
                            <div id="vista_previa_container" style="<?php echo count($horarios) > 0 ? 'display:block' : 'display:none'; ?>" class="mt-3">
...
<script>
// ============================================
// FUNCIONES PARA EL FORMULARIO DE EDICION
// ============================================

// Toggle del switch
function toggleSwitch() {
    var checkbox = document.getElementById('actualizar_horario');
...
// Inicializar contador y vista previa
...
document.getElementById('formMateria').addEventListener('submit', function(e) {
    var actualizarHorario = document.getElementById('actualizar_horario').checked;
